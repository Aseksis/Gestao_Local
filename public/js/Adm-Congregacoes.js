    ...
    linha = '<tr>' +
        ...
        '<button type="button"' + 'class="btn btn-sm btn-primary mr-2"' + 'onclick="editarCongregacao(' + c.id + ')"> Editar </button>' +

        '<button type="button"' + 'class="btn btn-sm " style="background:#ffe019; border-color:#ffe019;"' + 'onclick="viewCongregacao(' + c.id + ')"> Visualizar</button>' +
...
$('#paginator_congregacao>ul>li').remove();

$('#paginator_congregacao>ul').append(getItemAnterior(data));

for (i = 1; i <= data.last_page; i++) {
    s = getItem(data, i);
    $('#paginator_congregacao>ul').append(s);
}

$('#paginator_congregacao>ul').append(getItemProximo(data));
