...
linha = '<tr>' +
    ...
    '<button type="button"' + 'class="btn btn-sm btn-primary mr-1"' + 'onclick="editarRegional(' + r.id + ')"> Editar </button>' +
    '<button type="button"' + 'class="btn btn-sm btn-primary"  style="background:#e3342f; border-color:#e3342f;"' + 'onclick="viewRegional(' + r.id + ')"> <font color="#000000">Visualizar</font>  </button>' +
    '</div>' +
    '</td>'
...
$('#paginator_regional>ul>li').remove();

$('#paginator_regional>ul').append(getItemAnterior(data));

for (i = 1; i <= data.last_page; i++) {
s = getItem(data, i);
$('#paginator_regional>ul').append(s);
}

$('#paginator_regional>ul').append(getItemProximo(data));
